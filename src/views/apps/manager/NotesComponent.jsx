...
const NotesComponent = ({ 
  ...
  const fetchNotes = async (taskId) => {
    try {
        ...
        const normalized = data.map(raw => {
          // Better ID handling - use whichever field exists
          const id = raw._id || raw.id;
          if (!id) {
            console.error('Note missing ID:', raw);
          }
          return {
            _id: id, // Use _id as the standard field name
        ...
        console.log('Fetched notes with IDs:', normalized.map(n => ({ _id: n._id, id: n.id })));
        setNotes(normalized);
    } catch (error) {
  ...
  const addNote = async (taskId, content) => {
    ...
    try {
        ...
        const newNoteRaw = response.data.note || response.data;
        const newNote = {
          _id: newNoteRaw._id || newNoteRaw.id,
          id: newNoteRaw._id || newNoteRaw.id,
          content: newNoteRaw.content || '',
          createdAt: newNoteRaw.createdAt,
          updatedAt: newNoteRaw.updatedAt,
          createdBy: newNoteRaw.createdBy,
          updatedBy: newNoteRaw.updatedBy,
        };
        console.log('Added note with ID:', newNote._id);
  ...
  const updateNote = async (taskId, noteId, content) => {
    ...
    if (!noteId) {
      console.error('Cannot update note: noteId is undefined');
      setError('Cannot update note: Note ID is missing');
      return;
    }
    ...
    try {
        ...
        const updatedNote = {
          _id: raw._id || raw.id || noteId, // Fallback to original noteId
          id: raw._id || raw.id || noteId,
          content: typeof raw.content === 'string' ? raw.content : '',
          createdAt: raw.createdAt,
          updatedAt: raw.updatedAt,
          createdBy: raw.createdBy,
  ...
  const deleteNote = async (taskId, noteId) => {
    ...
    if (!noteId) {
      console.error('Cannot delete note: noteId is undefined');
      setError('Cannot delete note: Note ID is missing');
      return;
    }
  ...
  const startEditingNote = (note) => {
    console.log('Starting edit for note:', note); // Debug log
    console.log('Note ID:', note._id); // Debug log
    
    if (!note._id) {
      console.error('Cannot edit note: note._id is undefined', note);
      setError('Cannot edit note: Note ID is missing');
      return;
    }
