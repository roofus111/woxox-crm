...
const Dashboard = () => {
    ...
    const fetchDocuments = async () => {
        try {
            ...
            const docs = data.documents.map(doc => {
                ...
                return {
                    id: doc._id || '',
                    title: doc.docName || doc.title || 'Untitled',
                    content: doc.content || '',
                    fileUrl: doc.fileUrl || '',
                };
    ...
    // Function to upload the document as PDF to a lead
    const handleUploadToLead = async () => {
        if (!editorContent) {
            setError('No document content to upload');
            return;
        }
        ...
        try {
            ...
            // Generate PDF from the editor content
            const pdf = await exportToPdf(documentTitle || 'Document', editorContent);
            const pdfBlob = pdf.output('blob');

            // Create filename with timestamp to ensure uniqueness
            const filename = `${documentTitle || 'Document'}_${Date.now()}.pdf`;

            // Create form data for upload
            const formData = new FormData();
            formData.append('files', new File([pdfBlob], filename, { type: 'application/pdf' }));

            // Add leadId if available
            if (leadId) {
            ...
            if (response.data) {
                // Navigate back to the lead page if leadId is available, otherwise to the files page
                if (leadId) {
                    router.push(`/en/manager/leads/leadId=${leadId}`);
                } else {
                    router.push('/en/manager/myfiles');
                }
            }
        } catch (err) {
            console.error('Error uploading document to lead:', err);
            if (err.response && err.response.data && err.response.data.message) {
                setError(`Failed to upload document: ${err.response.data.message}`);
            } else if (err.response && err.response.data && err.response.data.error) {
                setError(`Failed to upload document: ${err.response.data.error}`);
            } else {
    ...
    const handleExportPdf = async () => {
        try {
            ...
            // Save the PDF
            pdf.save(`${documentTitle.replace(/\s+/g, '_') || 'document'}.pdf`);
                            ...
                            <div className="mt-6 flex space-x-4">
                                ...
                                {leadId && (
                                    <button
                                        ...
                                        disabled={loading}
                                    >
                                        {loading ? 'Uploading...' : 'Save & Upload to Lead'}
                                    </button>
                                )}
                            </div>
                        </div>
