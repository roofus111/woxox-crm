...
const TaskManager = ({ initialView = 'all-tasks' }) => {
  const { data: session } = useSession();
  const [currentView, setCurrentView] = useState(initialView || 'all-tasks');
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get(
        `${apiBase()}/api/user-profiles/users/active`,
        { headers: getHeaders() }
      );
  ...
  const updateTask = async (taskId, updatedData) => {
    try {
      ...
      const response = await axios.put(
        `${apiBase()}/api/tasks/updatetask/${taskId}`,
        updatedData,
        { headers: getHeaders() }
      );
  ...
  const deleteTask = async (taskId) => {
    try {
      ...
      await axios.delete(
        `${apiBase()}/api/tasks/deletetask/${taskId}`,
        { headers: getHeaders() }
      );
...
const fetchTaskById = async (taskId) => {
  try {
    ...
    setError(null);
    const response = await axios.get(
      `${apiBase()}/api/tasks/gettask/${taskId}`,
      { headers: getHeaders() }
    );
    setSelectedTask(response.data);
  ...
  const searchTasks = async (filters = searchFilters) => {
    try {
      ...
      const url = `${apiBase()}/api/tasks/search${queryString ? `?${queryString}` : ''}`;
...
const uploadFile = async (taskId, file) => {
  ...
  try {
    ...
    const response = await axios.post(
      `${apiBase()}/api/tasks/uploadfile/${taskId}`,
      formData,
      {
        headers: {
          ...getHeaders(),
...
const deleteFile = async (taskId, fileId, fileName) => {
  ...
  try {
    ...
    await axios.delete(
      `${apiBase()}/api/tasks/deletefile/${taskId}/${fileId}`,
      { headers: getHeaders() }
    );
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <div>
                    ...
                    >
                      {file.size ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : 'Size unknown'} â€¢ 
